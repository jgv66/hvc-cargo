        ...
        if (body.interno === undefined) {
            ...
            query = `
                ...
                SELECT pqt.id, pqt.id_paquete
                    ...
                    ,convert(nvarchar(5), pqt.fecha, 108) as hora
                    ,pqt.estado, pqt.comentario, pqt.usuario, usuario.nombre
					,(case when pqt.estado in ('950','900') then (case when elpqt.recep_nombre='' then 'no-registrado' else elpqt.recep_nombre end)
						   else '' end)	as receptor 
					,(case when pqt.estado in ('950','900') then (case when elpqt.recep_nombre='' then '' else elpqt.recep_rut end)
						   else '' end)	as rut_receptor  
        ...
        const query = `
            ...
            begin try 
                if exists ( select * 
                            ...
                            where p.id_paquete = ${ body.id_pqt } ) begin
                    --
                    update k_paquetes set valor_cobrado = ${ body.precio }
                           ,peso = ${ body.peso }, volumen = ${ body.volumen }
                           ,bultos = ${ body.bultos }
                    where id_paquete = ${ body.id_pqt };
                    --
